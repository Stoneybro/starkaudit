...
## Sealed threshold distribution: CODE-COMPLETE, pending redeploy (2026-09-05)
- **Why:** threshold + salt reached the business backend as hardcoded constants (`0xcafebabe` in-repo) — an offline DM, not an audit. Now sealed end-to-end; manual delivery is removed from code, UI copy, and scripts.
- **Contract** (`audit_registry.cairo`): `distribution_keys:Map<business,DistributionKey>` + `packages:Map<(business,version),ThresholdPackage>`, `set_distribution_key` open self-serve, `share_threshold_package` auditor-only bound to live version (`NO_THRESHOLD`/`NOT_AUDITOR`), `get_/has_` views, `DistributionKeySet`/`ThresholdPackageShared` events. **snforge 24/24** (6 new: key set/get, missing-key revert, package bound to version, non-auditor revert, no-threshold revert, missing-package revert).
- **Crypto** (`packages/audit-sdk/src/distribution.ts`, tweetnacl box): plaintext `threshold_u256_be32||salt_be32||version_u64_be8` (72B) → 88B ciphertext → 6 felts. `scripts/dist_roundtrip_check.ts` green: seal→open→poseidon reproduces live `0x7da138…`, wrong-key and tampered-ciphertext both rejected.
- **Auditor UI** (`apps/web` Tests tab): T1 save commits on-chain then auto-seals + shares to every keyed business (sequential, per-business status); Distribution section shows key/package state per business with Share/Resend/Share-all; salt is `randomFeltHex()` per rotation with Randomize button; (threshold, salt) live in browser-session memory only.
- **Backend:** `scripts/dist_keygen.ts` publishes pubkey (secret → `BUSINESS_DIST_SECRET`, `.env.example` updated), `scripts/sync_package.ts` decrypts + verifies version/commitment + writes gitignored `threshold-package.json`; `stage5.ts` reads it and refuses without it (`threshold-package.json missing — run sync_package first`). Live-checked: reads on-chain version 1, then fails clean with "auditor must share it" (see gate below).
- **Gate — NOT active on-chain:** deployed registry `0x1ce71384…` predates the new entrypoints (`get_duplicate_window` already falls back to "Unknown" in UI). **Redeploy checklist (needs approval):** declare + deploy new registry → `register_business` → `set_auditor` → `dist_keygen` → T1 commit via UI (auto-share) → `sync_package` verifies → re-run Stage 5 legs → update hardcoded `REGISTRY` in `registry_setup.ts`/`stage5.ts`/`sync_package.ts`/`dist_keygen.ts` + `apps/web/.env.local` (`NEXT_PUBLIC_AUDIT_REGISTRY`, `NEXT_PUBLIC_REGISTRY_DEPLOY_BLOCK`).
